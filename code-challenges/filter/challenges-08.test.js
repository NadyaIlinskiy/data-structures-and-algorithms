...
Write a function named filterStringsWithVowels that, given an array of strings as input, uses filter
to return an array with only words that contain vowels.
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  return arr.filter(el => /[aeiou]/.test(el));
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(el => el.children===undefined);
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(el => typeof(el)==='number'&& el.toString().match(/^\d+$/)).map(el => el%2 ===0 ? 'even' : 'odd');
};
