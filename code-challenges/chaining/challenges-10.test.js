...
const count = (target, input) => {
  // Solution code here...
  return input.reduce ((c, el) =>
    el.reduce((acc, x) => target===x ? acc+1 : acc, c)
  , 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(element => element.reduce((arr, el)=>{
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(el => el.gender==='female'||el.gender==='male').map(el => el.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((a ,b) => +a.height < +b.height ? a : b ).name;
