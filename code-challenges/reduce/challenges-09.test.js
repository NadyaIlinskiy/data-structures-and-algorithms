...
const reversedString = (str) => {
  // Solution code here...
  return str.split('').reduce((rts, char) => char+rts, '');
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((num, element) => num+ (typeof(element.children) !== 'undefined'? element.children.length: 0), 0);
};
...
const calculateAverage = (arr) => {
  ...
  let obj = arr.reduce((o, x)=> {return { count: o.count+1, sum: o.sum+x }},{ count: 0, sum: 0 })
...
Write a function named countPrimeNumbers that, given an array elements as input, uses reduce
to count the number of elements that are prime numbers.
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((acc, x) => acc + (isPrime(x)?1:0 ), 0);
};
